...
using AnimeDefendersEngine::TableReader;
...
[[nodiscard]] auto TableReader::endOfStream() const -> bool {
...
void TableReader::readNewRow() {
    ...
    auto splittedString = SplitString(line, m_wordsInRowSeparator);
...
[[nodiscard]] auto TableReader::operator[](const TableReader::keyType& key) const -> TableReader::valueType {
...
TableReader::TableReader(std::unique_ptr<std::istream>&& dataStream, const char wordsInRowSeparator)
    ...
    readNewRow();
    if (!is_ValuesReadable()) {
        throw std::runtime_error{"Unable to read first table row"};
    }
}

[[nodiscard]] auto TableReader::is_ValuesReadable() const -> bool {
